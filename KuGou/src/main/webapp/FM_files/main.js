...
var makeRightList = function(n,m){
	...
	if(m!=0){
	  ...
	  var data = Radio.JSON.stringify(dataObj);  
...
var sendData = function(fmInfo,num){
    ...
    var dataSendStr = Radio.JSON.stringify(fmInfo);	
...
var createSucc = function(type,value,fmtype,imgurl,offset,songs){
	...
	var data = Radio.JSON.stringify(sendStr);
	//console.log(data);
...
var createClick = function(){
	...
	var dataClick = Radio.JSON.stringify(dataObj);  
...
function listenHistory(){
   ...
   //alert(history);
   var hisobj = Radio.JSON.parse(historys);
...
var shareLink = function(url){
	...
	if(paras.indexOf("&") != -1) {
		...
		var re = /^[0-9]*[1-9][0-9]*$/;
		if(re.test(fmid)) {
		  var linkfmData = radioData.Clone();
		  if(fmtype == 2) {
