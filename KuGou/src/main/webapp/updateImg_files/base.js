...
(function (d,w){
		...
		monitor(resources, function(args,nosupport) {
			for (var i = 0, len = args.length; i < len; i++) {
				...
				var ex = src && src.match("^(http|https):\/\/(.*?)\/"),
					falg = false;
				//劫持变种类型
				//0：非iframe -2：未知变种iframe,1\2\3：变种，正常iframe：11
				var htype = 0; 
				if (name == "iframe") {
					htype = -2;
				}
				...
				//javascript:;为正常
				if (!falg && name == 'iframe' && src && src == "javascript:;") {
					htype = 11;
				}
				//3、iframe变种3
				if (!falg && name == 'iframe') {
				...
				if (falg) {
					...
					//删除资源(未知先不删)
					if (hijackedCur.open && /\bstyle\b|\bscript\b|\blink\b/.test(name) == false && htype != -2) {
						removeRes(res);
					}
				}
			}
	...
	function sendhijackedData(jsUrl, jsUrl2, jsContent) {
		getUserInfo(function(uid){
			...
			if (!resources.src && !resources.iframe && !resources.hc) {
				hijackedResObj = null;
				return;
			}
			if(ver >= 7700){
